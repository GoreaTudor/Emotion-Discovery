...
all_utterances = pd.concat([all_utterances, our_testing_csv["Utterance"]], ignore_index=True)

all_emotions = our_training_csv["Emotion_name"]
all_emotions = pd.concat([all_emotions, our_testing_csv["Emotion_name"]], ignore_index=True)

all_speakers = our_training_csv["Speaker"]
all_speakers = pd.concat([all_speakers, our_testing_csv["Speaker"]], ignore_index=True)

all_annotations = our_training_csv["Annotate(0/1)"]
all_annotations = pd.concat([all_annotations, our_testing_csv["Annotate(0/1)"]], ignore_index=True)
...
for utt in idx2utt.values():
    ...
    if pp_utt != "pad":
      weight_matrix[utt2idx[pp_utt]] = generate_random_float()
